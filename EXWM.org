...
*** Function to select or launch Google Chat
    #+begin_src emacs-lisp
      (defun gjg/exwm-select-or-launch-google-chat ()
        ...
        (interactive)
        (gjg/exwm-switch-to-buffer-or-launch "chat.google.com" "google-chat"))
    #+end_src
...
*** Function to switch quickly to Microsoft Teams in Edge
    ...
    #+end_src
*** Function to switch quickly to homelab Gitea page
    #+begin_src emacs-lisp
      (defun gjg/exwm-select-or-launch-gitea ()
        "Select the running Gitea window if it's running, otherwise launch."
        (interactive)
        (gjg/exwm-switch-to-buffer-or-launch "gitea.magichome" "gitea"))
    #+end_src
...
** Select apps such as secondary Emacs to start in char-mode, start floating/tiled etc.
   ...
   #+begin_src emacs-lisp
     (setq exwm-manage-configurations
           '(((member exwm-class-name '("Emacs" "Gnome-terminal" "konsole" "libreoffice-calc" "xdg-desktop-portal-gnome" "kitty" "UXTerm" "com.mitchellh.ghostty"))
              char-mode t)
...
** Start wallpaper with transparency first for aesthetics
   ...
   #+begin_src emacs-lisp
     ...
     (require 'exwm-randr)
   #+end_src
** Exit the controlling EXWM session directly
   Emacs normally binds =C-x C-c= to =save-buffers-kill-terminal=, which delegates
   to =save-buffers-kill-emacs= on a non-client frame.  EXWM advises the former
   command and adds a redundant terminal-kill confirmation before that delegation.

   On the controlling EXWM frame, exit the Emacs process directly.  Preserve the
   standard behavior for real emacsclient frames so =C-x C-c= closes only that
   client connection.

   #+begin_src emacs-lisp
     (defun gjg/exwm-save-buffers-kill (&optional arg)
       "Save buffers and exit EXWM, or close an Emacs client frame.
With prefix ARG, silently save all file-visiting buffers."
       (interactive "P")
       (if (frame-parameter nil 'client)
           (save-buffers-kill-terminal arg)
         (save-buffers-kill-emacs arg)))

     (global-set-key (kbd "C-x C-c") #'gjg/exwm-save-buffers-kill)
   #+end_src
      ...
      (defun gjg/exwm-config ()
        ...
        (setq exwm-input-global-keys
              `(
                ...
                (,(kbd "s-SPC") . xdg-launcher-run-app)
                ...
                ([?\s-g] . gjg/exwm-select-or-launch-gitea)
                ...
                ([?\s--] . browser-gt-tab-jump)
                ([?\s-_] . browser-gt-tab-manager)
...
#+begin_src emacs-lisp
  (use-package xdg-launcher
    :load-path "~/projects/emacs/xdg-launcher/")
#+end_src
...
*** Deal with displays
   ...
   [[https://www.reddit.com/r/emacs/comments/j0zda7/exwm_randr_config_for_laptop_disconnect/?utm_source=share&utm_medium=web2x&context=3][post on /r/emacs]]
	 ...
	 (setq exwm-randr-workspace-monitor-plist '(0 "HDMI-1" 1 "eDP-1")) ;; HDMI-1 is the big monitor, if connected
	 ...
	 (exwm-randr-mode 1)
	 ;; (unless exwm-randr--connection
	 ;;   (progn
	 ;;     (message "Strike 1: exwm-randr--connection is nil right after setting exwm-randr-mode.  Sleepytime")
	 ;;     (sleep-for 1)
	 ;;     (unless exwm-randr--connection
	 ;;       (progn
	 ;;         (message "Strike 2: exwm-randr--connection is nil after 2 sleep seconds.  Once more we sleep")
	 ;;         (sleep-for 1)
	 ;;         (unless exwm-randr--connection
	 ;;           (progn
	 ;;             (message "Strike 3: exwm-randr--connection is a no-show")))))
	 ;;     ))
	 ;; (exwm-randr-refresh)
                   ...
                   (lambda ()
                     (let ((browsers '(
                       ...
                       (cond
                        ...
                        ((string= "com.mitchellh.ghostty" exwm-class-name)
                         (exwm-workspace-rename-buffer (concat "ghostty - " exwm-title)))
                        ((string= "Slack" exwm-class-name)
                         (exwm-workspace-rename-buffer (concat "slack" "- " exwm-title)))
