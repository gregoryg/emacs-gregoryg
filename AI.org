...
*** Image generation tool
    #+begin_src emacs-lisp
      ...
      (defun gjg/generate-image-tool (prompt)
        "Generate an image from PROMPT using OpenRouter and save it locally.

      Return a user-facing status string that includes the saved path and
      an Org mode file link."
        (let* ((openrouter-backend
          ...
          (unless (and api-key (not (string-empty-p api-key)))
            (user-error "Image generation failed: could not resolve OpenRouter API key"))
          (let* ((url-request-method "POST")
                 ...
                 (response-buffer
                  (url-retrieve-synchronously
                   "https://openrouter.ai/api/v1/chat/completions" t t 60)))
            (unless response-buffer
              (user-error "Image generation failed: no response from OpenRouter"))
            (unwind-protect
                (with-current-buffer response-buffer
                  ...
                  (let* ((response (gptel--json-read))
                    ...
                    (unless image-bytes
                      (user-error
                       "Image generation failed: response did not contain image data"))
      ...
      (gptel-make-tool
       ...
       :variables '((prompt :type string :description "The detailed visual prompt for image generation"))
       :fn #'gjg/generate-image-tool
       :category "media")
    #+end_src
