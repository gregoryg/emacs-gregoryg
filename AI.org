...
*** Flashcard examples for Portuguese
    #+begin_src emacs-lisp
      (gptel-make-preset 'portuguese-sample-sentences
        :description "Generate sample sentences based on portuguese flashcard words or phrases" :backend
        "ChatGPT Codex - sub" :model 'gpt-5.6-terra :system
        "please give 4-5 memorable portuguese sentences using the portuguese terms, with translations in Spanish.  simple list, no bolding or emphasis\n"
        :tools 'nil :stream t :temperature nil :max-tokens nil :use-context 'system :track-media t
