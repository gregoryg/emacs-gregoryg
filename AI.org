...
**** Web / Internet / Online Research category
    ...
    Let's load that sucker now:
    #+begin_src emacs-lisp
      (with-eval-after-load 'gptel
        (load-file "~/projects/emacs/gregoryg-packages/gptel-native-tools/gptel-native-shell.el"))
#+end_src
...
*** File system
    ...
    Load the library, then add a function to gptel-mode-hook to automatically add the CWD and (optionally) project.el root to the buffer-local list of allowed directories

    #+begin_src emacs-lisp
      (with-eval-after-load 'gptel
          (load-file "~/projects/emacs/gregoryg-packages/gptel-native-tools/gptel-native-filesystem.el")
...
**** Search the web and read a web page
      ...
      NOTE: These tools have now been moved to ~/projects/emacs/gregoryg-packages/gptel-native-tools/gptel-native-web.el

      #+begin_src emacs-lisp
(load "~/projects/emacs/gregoryg-packages/gptel-native-tools/gptel-native-web.el")
      #+end_src
