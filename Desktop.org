...
** X Session Desktop file for Lightdm
   ...
   #+end_src
** OpenSSH agent for all user sessions
   Use the OpenSSH agent supplied by =openssh-client= as a socket-activated
   =systemd --user= service.  TTY, EXWM, and conventional desktop sessions all
   use the stable socket at =$XDG_RUNTIME_DIR/openssh_agent=.  Agent keys expire
   after eight hours by default.

   Do not start agents from Emacs or =keychain=.  The GCR and GnuPG packages may
   remain installed for GNOME and GPG, but their competing SSH-agent sockets are
   masked for this user.

   #+begin_src conf :tangle ~/.config/systemd/user/ssh-agent.service.d/override.conf
     [Service]
     ExecStart=
     ExecStart=/usr/bin/ssh-agent -D -t 8h
   #+end_src

   #+begin_src conf :tangle ~/.config/environment.d/60-ssh-agent.conf
     # Authentication agent for services launched by systemd --user.
     SSH_AUTH_SOCK=${XDG_RUNTIME_DIR}/openssh_agent
   #+end_src

   Preserve an agent forwarded into an SSH login; otherwise use the local
   socket-activated agent.  =SSH_AGENT_PID= is intentionally absent because
   clients communicate through the stable socket rather than a daemon PID.

   #+begin_src sh :tangle ~/.config/ssh-agent-env.sh
     # Use the socket-activated OpenSSH agent unless this session has a live
     # forwarded agent of its own.
     if [ -z "${SSH_AUTH_SOCK:-}" ] || [ ! -S "$SSH_AUTH_SOCK" ]; then
         SSH_AUTH_SOCK="${XDG_RUNTIME_DIR:-/run/user/$(id -u)}/openssh_agent"
         export SSH_AUTH_SOCK
     fi

     # A socket-activated agent has no stable PID for clients to track.
     unset SSH_AGENT_PID
   #+end_src

   Login shells source the fragment from =~/.bash_profile=.  Debian Xsession
   sources =~/.xsessionrc= before its optional SSH-agent wrapper.

   #+begin_src sh :tangle ~/.xsessionrc
     # Give X11 sessions the same socket-activated OpenSSH agent used by TTY and
     # systemd user sessions.  This is sourced before Debian's optional Xsession
     # ssh-agent wrapper.
   ...
   #+begin_src bash
     systemctl --user mask --now gcr-ssh-agent.service gcr-ssh-agent.socket gpg-agent-ssh.socket
     systemctl --user enable --now ssh-agent.socket
     sudo sed -i '/^use-ssh-agent$/d' /etc/X11/Xsession.options
     sudo apt purge keychain
   #+end_src
...
** Amazon.com
   ...
   #+end_src
** ArgoCD
      #+begin_src conf :tangle ~/.local/share/applications/argocd.desktop
        [Desktop Entry]
        Name=ArgoCD for k3s
        Comment=ArgoCD for homelab
        Exec=xdg-open https://argocd.magichome/
        Icon= /home/gregj/.local/share/pixmaps/argocd-original-512x512.png
        Terminal=false
        Type=Application
        Categories=Devops;CICD;Kubernetes;
   #+end_src
...
** Bartender's Friend
   + generic browser, specified by update-alternatives x-www-browser *or* xdg-settings
   #+begin_src conf :tangle ~/.local/share/applications/bartenders-friend.desktop
     [Desktop Entry]
     Name=Bartender's Friend
     Comment=
     Exec=xdg-open https://bartenders-friend.magichome/
     Icon= /home/gregj/.local/share/pixmaps/bartenders-friend-mottled-01-512x512.png
     Terminal=false
     Type=Application
     Categories=Fun;Web;Recipes
   #+end_src
** Canvas Credit Union
...
** Cockpit / Red Hat Web Console
   ...
   #+end_src
** Cryptogrammer
   #+begin_src conf :tangle ~/.local/share/applications/cryptogrammer.desktop
          [Desktop Entry]
          Name=Cryptogrammer
          Comment=Substitution cipher helper app
          Exec= xdg-open https://cryptogrammer.magichome/
          Icon= /home/gregj/.local/share/pixmaps/cryptogrammer-01-512x512.png
          Terminal=false
          Type=Application
          Categories=Development;Coding;Homelab
   #+end_src
** Dired
...
** Erebor Synology
   #+begin_src conf :tangle ~/.local/share/applications/erebor.desktop
     [Desktop Entry]
     Name=Erebor Synology DSM NAS
     Comment=Erebor
     Exec=xdg-open https://nas.magichome
...
** Excalidraw -
   #+begin_src conf :tangle ~/.local/share/applications/excalidraw.desktop
          [Desktop Entry]
          Name=Excalidraw
          Comment=
          Exec= xdg-open https://excalidraw.magichome/
          Icon= /home/gregj/.local/share/pixmaps/excalidraw-logo-512x512.png
          Terminal=false
          Type=Application
          Categories=Development;Coding;Homelab
   #+end_src
...
** Gitea - self-hosted github
   #+begin_src conf :tangle ~/.local/share/applications/gitea.desktop
          [Desktop Entry]
          Name=Gitea
          Comment=Self-hosted github
          Exec= xdg-open https://gitea.magichome/
          Icon= /home/gregj/.local/share/pixmaps/gitea-logo-512x512.png
          Terminal=false
          Type=Application
          Categories=Development;Coding;Homelab
   #+end_src
** GitHub
   #+begin_src bash  :tangle ~/.local/share/applications/github.desktop
     ...
     Icon= /home/gregj/.local/share/pixmaps/github-11-xxl.png-512x512.png
     Terminal=false
     Type=Application
     Categories=Development;Coding
   #+end_src
** Google AI Studio - Gemini Dashboard
   #+begin_src bash  :tangle ~/.local/share/applications/ai-studio.desktop
     [Desktop Entry]
     Name=Google Gemini AI Studio
     Comment=Gemini and Nano Banana
     Exec=xdg-open https://aistudio.google.com/usage
     Icon= /home/gregj/.local/share/pixmaps/google-ai-studio-logo-1.png
     Terminal=false
     Type=Application
...
** Gotify notifications
   #+begin_src bash  :tangle ~/.local/share/applications/gotify.desktop
     [Desktop Entry]
     Name=Gotify notifications
     Comment=
     Exec=xdg-open https://gotify.magichome/
     Icon= /home/gregj/.local/share/pixmaps/gotify-logo2.png-512x512.png
     Terminal=false
     Type=Application
     Categories=Notification;Messaging;Self-hosted
   #+end_src
** Hangouts / Google Chat
   ...
   #+end_src
** Hermes Agent UI
   #+begin_src conf :tangle ~/.local/share/applications/hermes-agent-ui.desktop
     [Desktop Entry]
     Name=Hermes AI Agent Web UI
     Comment=Local Hermes
     Exec=xdg-open https://hermes-ui.magichome/
     Icon= /home/gregj/.local/share/pixmaps/hermesagent.png
     Terminal=false
     Type=Application
     Categories=AI;LLM
   #+end_src
...
** Home Assistant
   #+begin_src conf :tangle ~/.local/share/applications/home-assistant.desktop
     ...
     Exec=xdg-open https://homeassistant.magichome
...
** Jellyfin media server
   #+begin_src conf :tangle ~/.local/share/applications/jellyfin.desktop
     ...
     Exec=xdg-open https://jellyfin.magichome
...
** Google Calendar
   ...
   #+end_src
** Google Gmail - Dynapse
   #+begin_src conf :tangle ~/.local/share/applications/gmail-dynapse.desktop
     [Desktop Entry]
     Name=Gmail Dynapse Mail
     Comment=Google EMail in Browser
     Exec=xdg-open https://mail.google.com/mail/u/1/#search/in:inbox+is:unread
...
** Monitor change
   ...
   #+end_src
** N8n workflow thing
   #+begin_src conf :tangle ~/.local/share/applications/n8n.desktop
          [Desktop Entry]
          Name=N8n
          Comment=Workflow thingie
          Exec= xdg-open https://n8n.magichome/
          Icon= /home/gregj/.local/share/pixmaps/n8n-color-512x512.png
          Terminal=false
          Type=Application
          Categories=Development;Workflow;Automation
   #+end_src
...
** Node RED workflow thing
   #+begin_src conf :tangle ~/.local/share/applications/n8n.desktop
          [Desktop Entry]
          Name=Node RED
          Comment=Workflow thingie
          Exec= xdg-open https://node-red.magichome/
          Icon= /home/gregj/.local/share/pixmaps/node-red-hexagon-512x512.png
          Terminal=false
          Type=Application
          Categories=Development;Workflow;Automation
   #+end_src
...
** Lemmy
   ...
   #+end_src
** Patreon
   #+begin_src conf :tangle ~/.local/share/applications/patreon.desktop
     [Desktop Entry]
     Name=Patreon
     Comment=
     Exec=xdg-open https://patreon.com
     Icon= /home/gregj/.local/share/pixmaps/patreon-logo-png-transparent.png-512x512.png
     Terminal=false
     Type=Application
     Categories=Stuff
   #+end_src
** Reddit
   #+begin_src conf :tangle ~/.local/share/applications/reddit.desktop
     [Desktop Entry]
     Name=Reddit
     Comment=Reddit!
     Exec=xdg-open https://reddit.com/
     Icon= /home/gregj/.local/share/pixmaps/emotionless-reddit-512x512.png
     Terminal=false
     Type=Application
...
** Stable Diffusion
   ...
   #+end_src
** Gortsleigh AWS Console
   #+begin_src conf :tangle ~/.local/share/applications/stardog-aws-console.desktop
     [Desktop Entry]
     Name=Amazon Web Services Console - Gortsleigh Dynapse
     Comment=
     Exec=xdg-open https://dynapse.signin.aws.amazon.com/console
...
** Ubiquiti Unifi Site Manager
    ...
    #+begin_src conf :tangle ~/.local/share/applications/ubiquiti-unifi.desktop
      [Desktop Entry]
      Name=Ubiquiti Unifi Site Manager
      Comment=
      Exec=xdg-open https://unifi.ui.com/
      Icon= /home/gregj/.local/share/pixmaps/ubiquiti-unifi-01.png
      Terminal=false
      Type=Application
...
** YouTube
   ...
   #+end_src
** YouTube - My Channel
   #+begin_src conf :tangle ~/.local/share/applications/youtube-channel.desktop
     [Desktop Entry]
     Name=YouTube Channel
     Comment=
     Exec=xdg-open https://studio.youtube.com/channel/UCiJCWrG9_BFY5hwH3D-2D-Q
     Icon= /home/gregj/.local/share/pixmaps/rock-star-clipart-md-512x512.png
     Terminal=false
     Type=Application
     Categories=Video;yt
   #+end_src
...
** Yummy Sauce - mealie
   #+begin_src conf :tangle ~/.local/share/applications/yummy-sauce.desktop
     ...
     Comment=
     Exec=xdg-open https://yummy-sauce.magichome/
...
#+end_src
   ...
   #+begin_src conf :tangle ~/.local/share/applications/zen-browser.desktop
     ...
     Icon= /home/gregj/.local/share/pixmaps/zen-browser-logo-colorful-512x512.png
     Terminal=false
     Type=Application
