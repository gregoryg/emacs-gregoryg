...
** Laptop battery level notifications
   #+begin_src bash :tangle ~/bin/notify-battery-levels.sh
     ...
     if [[ $CABLE_PLUGGED == 'yes' ]]; then
         if [[ $BATTERY_PERCENTAGE -gt ${TARGET_MAX} ]]; then
         ...
         if [[ $BATTERY_PERCENTAGE -lt ${TARGET_MIN} ]]; then
             /usr/bin/notify-send --urgency=critical --icon=${ICON_LOW} "Battery optimization" "Battery is below ${TARGET_MIN}%, plug in the power cable to optimize battery life."
         fi
     fi
   #+end_src
* OS Applications
...
* Python virtual environment including Gmail packages
  ...
  #+begin_src bash :results both
    python3 -m venv ~/.local/python-venvs/jupyter/
    source ~/.local/python-venvs/jupyter/bin/activate
    pip install google google_auth google_auth_oauthlib googleapi
    deactivate
    # Test with
    # ~/.local/python-venvs/jupyter/bin/python3 ~/.config/polybar/gmail-unread.py

  #+end_src
* Applications not handled by OS package managers
   #+end_src
...
** Browser
   ...
   #+begin_src conf :tangle ~/.local/share/applications/browser-generic.desktop
     ...
     Exec=xdg-open https://
     Icon= /home/gregj/.local/share/pixmaps/browser-mashup-icon.png
     Terminal=false
     Type=Application
     Categories=Fun;Web
   #+end_src
...
** Databricks VGTEST
   #+begin_src conf :tangle ~/.local/share/applications/cloudflare.desktop
     [Desktop Entry]
     Name=Databricks vgtest
     Comment=
     Exec=xdg-open https://adb-8799800888063735.15.azuredatabricks.net/?o=8799800888063735
     Icon= /home/gregj/.local/share/pixmaps/databricks-logo.png
     Terminal=false
     Type=Application
     Categories=Spark
   #+end_src
...
** Google Cloud Platform
   #+begin_src bash  :tangle ~/.local/share/applications/gcp.desktop
     ...
     Comment=GCP
     Exec=xdg-open https://console.cloud.google.com/compute/instances?authuser=2
     Icon= /home/gregj/.local/share/pixmaps/google-cloud-logo.png
     Terminal=false
     Type=Application
...
** Google Calendar
   #+begin_src conf :tangle ~/.local/share/applications/gcalendar.desktop
     ...
     Exec=xdg-open https://calendar.google.com/calendar/u/2/
...
** Google Gmail
   #+begin_src conf :tangle ~/.local/share/applications/gmail.desktop
     ...
     Exec=xdg-open https://mail.google.com/mail/u/2/#search/in:inbox+is:unread
...
** Microsoft Teams
   ...
   #+begin_src conf :tangle ~/.local/share/applications/teams-browser.desktop
     ...
     Comment=
     Exec=google-chrome https://teams.microsoft.com
...
** Rocket Language
   ...
   #+begin_src conf :tangle ~/.local/share/applications/rocket-language.desktop
     ...
     Comment=Spanish Portugues
     Exec=google-chrome https://app.rocketlanguages.com/
