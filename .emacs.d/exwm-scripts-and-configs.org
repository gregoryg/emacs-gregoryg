...
** OpenSUSE Tumbleweed
   #+begin_src bash
     sudo zypper install setxkbmap numlockx xmodmap picom feh xfce4-panel xfconf albert parcellite NetworkManager-applet blueman xfce4-whiskermenu-plugin xfce4-power-manager-plugin xfce4-mount-plugin xfce4-pulseaudio-plugin xfce4-screenshooter-plugin xfce4-statusnotifier-plugin git mlocate the_silver_searcher kubernetes-client

   #+end_src
** Ubuntu
   #+begin_src bash
     sudo apt update && sudo apt -y install albert silversearcher-ag mlocate libgccjit gcc libgccjit-9-dev vlc gwenview fonts-hack wmctrl feh compton autorandr fortune-mod cowsay ruby xscreensaver pdsh discount git
     sudo gem install hcl  # Harvest time tracking CLI
   #+end_src
