...
** Ubuntu
   #+begin_src bash
     sudo apt update && sudo apt -y install albert silversearcher-ag mlocate libgccjit gcc libgccjit-9-dev vlc gwenview fonts-hack wmctrl feh compton autorandr fortune-mod cowsay ruby xscreensaver pdsh discount
