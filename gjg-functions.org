    ...
    #+begin_src emacs-lisp
      (defun gjg/display-agenda-context (w)
        ...
        (when (string= (buffer-name) " *Agenda Commands*")
          (setq mode-name
                (concat "Fundamental[" gjg/current-agenda-context "]"))))
...
*** Create a context switcher
    #+begin_src emacs-lisp
      (defun gjg/select-agenda-context ()
        ...
        (let ((selection (completing-read "Ohai: " gjg/agenda-contexts-alist )))
          (setq gjg/current-agenda-context selection)
        (setq org-agenda-files
              (cadr (assoc (intern selection) gjg/agenda-contexts-alist)))))
