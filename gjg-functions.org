...
* Org Mode to RDF and (maybe) RDBMS
  [[id:6a76fbd7-4f5a-48a6-8b27-ca158946a7ca][make Org and Markdown files into RDF triples]]
...
** Code to return (very) simplifed headline parse of current Org buffer
   ...
   #+begin_src emacs-lisp
     ...
     (defun gjg/parse-org-buffer-for-rdf ()
       "Parse current Org buffer, returning elements for my ontology."
       (org-element-map (org-element-parse-buffer 'headline) 'headline
         (lambda (hl)
           (let* ((element-type (car hl))
                  (body (cadr hl))
                  (pos (plist-get body :robust-begin))
                  (properties (org-entry-properties pos))
                  (title (plist-get body :raw-value))
                  (level (plist-get body :level))
                  (todo-state (plist-get body :todo-type))
                  (parent (plist-get body :parent))
                  (parent-id (plist-get (cadr parent) :ID))
...
** Write RDF as turtle for directory of Org files
   #+begin_src emacs-lisp
     (defun gjg/write-org-to-rdf ()
       "Build RDF and write TURTLE file for all Org Mode files in current directory"
       (interactive)
         ...
         (dolist (file files)
           ...
           (let ((buffer (or (find-buffer-visiting file) (find-file-noselect file))))
             (with-current-buffer buffer
               ...
               (f-write (gjg/generate-rdf-from-org) 'utf-8 (concat "/tmp/" (file-name-sans-extension (file-name-nondirectory file)) ".ttl")
                        )
             )))
         ))
   #+end_src
